...
describe("ComptrollerOnline", () => {

    before(async function () {
        //await deployments.fixture(['ComptrollerUpdateG7', 'ClaimLens', 'Breeder']);
        ...
        tx = await tx.wait();
        this.claimLens = await ethers.getContract('ClaimLens');

        this.depToken = await ethers.getContract('WePiggyToken');
        this.DEP = this.depToken.address;
        this.masterChef = await ethers.getContract('PiggyBreeder');
        this.mockToken0 = await ethers.getContract('MockToken0');
        this.mockToken1 = await ethers.getContract('MockToken1');
        */
        this.depToken = await ethers.getContractAt('CErc20', '0x4E14A3EBb4d76f6f7010c6566B1fb1e52bBA4d0d');
    });
    ...
    it("ClaimOneByOne", async function() {
        let accountAddress = '0x20b51ad53de18b866467eec39b4c0d7c3d823a3a';
        await network.provider.request({
            method: "hardhat_impersonateAccount",
            params: [accountAddress],
        });
        let balanceBefore = await this.depToken.balanceOf(accountAddress);
        let account = await ethers.getSigner(accountAddress)
        tx = await this.comptroller.connect(account).claimAll('0x20b51ad53de18b866467eec39b4c0d7c3d823a3a', ['0xB95952B833d1f4e78080aA860E6cc99968178914', '0x26b345b7899Bf4Be44063301437F62CC4E3AcAe1', '0xE4dF556Ab7BE80bd01237c031B32EaA29974805C', '0x0fD3151b3A92E49F2b576dfdc7a7C591969FB4fb', '0x91d2398f412663E4d5433c589Eab9a36100AdADf', '0xe74D6AdbEB8115A8c9B663B65C8fe19db298f6A8']);
        tx = await tx.wait();
        console.dir(tx);
        let balanceAfter = await this.depToken.balanceOf(accountAddress);
        console.log("claim DEP: ", balanceAfter.sub(balanceBefore));
        /*
